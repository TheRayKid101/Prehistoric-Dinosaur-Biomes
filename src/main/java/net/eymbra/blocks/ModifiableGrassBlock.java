...
public class ModifiableGrassBlock extends ModifiableSpreadableBlock implements Fertilizable {
	public ModifiableGrassBlock(AbstractBlock.Settings settings) {
		super(settings.ticksRandomly().strength(0.6F).sounds(BlockSoundGroup.GRASS));
	}
	...
	public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
		...
		// blockPos2 = blockPos2.add(random.nextInt(3) - 1, (random.nextInt(3) - 1) *
		// random.nextInt(3) / 2, random.nextInt(3) - 1);
		// if (!world.getBlockState(blockPos2.down()).isOf(this) ||
		...
		// ((Fertilizable) blockState.getBlock()).grow(world, random, blockPos2,
		// blockState2);
		// }
		//
		...
		// world.getBiome(blockPos2).getGenerationSettings().getFlowerFeatures();
		// if (list.isEmpty()) {
		// continue;
		// }
		//
		...
		// blockState4 = flowerFeature.getFlowerState(random, blockPos2,
