...
public class ModifiedSaplingBlock extends PlantBlock implements Fertilizable {
	...
	protected ModifiedSaplingBlock(SaplingGenerator generator, AbstractBlock.Settings settings) {
		super(settings.noCollision().ticksRandomly().breakInstantly().sounds(BlockSoundGroup.GRASS));
	...
	protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
		return floor.isOf(Blocks.GRASS_BLOCK) || floor.isOf(Blocks.DIRT) || floor.isOf(Blocks.COARSE_DIRT) || floor.isOf(Blocks.PODZOL) || floor.isOf(Blocks.FARMLAND) || floor.isOf(EymbraBlocks.PREHISTORIC_GRASS_BLOCK)
				|| floor.isOf(EymbraBlocks.PREHISTORIC_DIRT_BLOCK);
	}
