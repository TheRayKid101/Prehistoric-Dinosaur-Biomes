...
public abstract class ModifiableSpreadableBlock extends SnowyBlock {
	protected ModifiableSpreadableBlock(AbstractBlock.Settings settings) {
		super(settings);
	}
	...
	public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
		if (!canSurvive(state, world, pos)) {
			world.setBlockState(pos, EymbraBlocks.PREHISTORIC_DIRT_BLOCK_STATE);
		} else {
			if (world.getLightLevel(pos.up()) >= 9) {
				...
				for (int i = 0; i < 4; ++i) {
					...
					if (world.getBlockState(blockPos).isOf(EymbraBlocks.PREHISTORIC_DIRT_BLOCK) && canSpread(blockState, world, blockPos)) {
